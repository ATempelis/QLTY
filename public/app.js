            ...
            categorizedTasks[category].forEach((task) => {
                ...
                showContentsButton.addEventListener("click", async (event) => {
                    ...
                    if (contentsList.classList.contains("hidden")) {
                            ...
                            .map((item) => {
                                if (item.isFolder) {
                                    ...
                                    return `<li><a href="/tasks/${encodeURIComponent(
                                        task.id
                                    )}/${encodeURIComponent(item.name)}" target="_blank">${item.name}</a></li>`;
                                }
                            })
                            .join("");
                        ...
                        const folderSelect =
                            uploadContainer.querySelector(".folderSelect");
                ...
                uploadForm.addEventListener('submit', async (event) => {
                    ...
                    try {
                        let response = await fetch(`/tasks/${encodeURIComponent(selectedFolder)}/upload`, {
                            method: 'POST',
                            body: formData
                        });
                
                        if (response.status === 409) { // File already exists
                            ...
                            if (overwrite) {
                                ...
                                response = await fetch(`/tasks/${encodeURIComponent(selectedFolder)}/upload`, {
                                    method: 'POST',
                                    body: formData
                                });
                            } else {
                        ...
                        toggle.addEventListener("click", async () => {
                            ...
                            if (subfolderContents.classList.contains("hidden")) {
                                if (!subfolderContents.hasChildNodes()) {
                                    ...
                                    const subfolderItems = await fetchTaskContents(
                                        fullPath
                                    );
                                    ...
                                    if (Array.isArray(subfolderItems)) {
                                        ...
                                        addFolderToggleListeners(
                                            subfolderContents,
                                            fullPath
                                        ); // Recursively add listeners for nested subfolders
                                    } else {
                                        console.error(
